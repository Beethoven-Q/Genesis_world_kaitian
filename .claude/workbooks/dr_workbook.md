...
| **pen** (dry-erase marker) | ⚠️ **NATIVE TEXTURE NEEDED** (currently DEGENERATE — flat colour stick) | should be a real marker texture (barrel colour + cap + label/branding) on a UV-mapped mesh | **none** (texture IS the look) | **A pure-colour stick is NOT a recognizable marker** (owner 2026-06-20). `dry_erase_marker_clean.obj` has **0 `vt`** (no UVs). **ACTION: re-export UV + bake the YCB marker texture (or source a textured marker).** |
